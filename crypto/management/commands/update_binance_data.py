...
class Command(BaseCommand):
    help = 'Fetches and updates Binance currency data'
    ...
    def update_currency_price(self, currency, trade_price):
        Prices.objects.filter(currencies=currency).order_by('currency_id').update(
            binance_last_trade_price=trade_price
        )
    ...
    async def fetch_all_prices(self):
        async with aiohttp.ClientSession() as session:
            ...
            for currency in currency_list:                
                ...
                task = asyncio.create_task(self.get_binance_price(session, currency))
                tasks[currency] = task
            ...
            for currency in currency_list:
                ...
                if len(results) > 0  and 'symbol' in results[currency] :
                    trade_price = results[currency]["price"]  # Last trade price
